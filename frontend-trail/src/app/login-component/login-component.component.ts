...
export class LoginComponentComponent {
  ...
  login() {
    ...
    this.http.post<any>('http://localhost:8082/Whospitals/user/auth/login', {
      email: this.email,
      password: this.password
    }, { headers }).subscribe({
      next: (response) => {
        ...
        this.authService.setUserId(response.userId || null);
        this.authService.setPatientId(response.patientId || null);

        if (response.doctorId) {
          this.authService.setUserRole('DOCTOR');
          this.router.navigate(['/doctor-dashboard']);
        } else if (response.patientId) {
          this.authService.setUserRole('PATIENT');
          this.router.navigate(['/patient-dashboard']);
        } else {
          alert('Unknown role. Please contact support.');
        }
